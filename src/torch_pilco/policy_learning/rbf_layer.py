...
class RBFLayer(nn.Module):
    ...
    def __init__(self,
        ...
        num_kernels: int,
        out_features_dim: int,
        radial_function: Callable[[torch.Tensor], torch.Tensor]=lambda x: torch.square(-x).exp(),
        norm_function: Callable[[torch.Tensor], torch.Tensor]=lambda x: torch.norm(x, p=2, dim=-1),
    ...
    def forward(
        ...
        diff = observation.view(batch_size, 1, self.in_features_dim) - c
        ...
        rbfs_dropped = self.f_drop(rbfs, p_dropout)
        # Take linear combination
        action = self.weights.expand(
            batch_size,
            self.out_features_dim,
            self.num_kernels
        ) * rbfs_dropped.view(batch_size, 1, self.num_kernels)

        return self.f_squash(action.sum(dim=-1))
